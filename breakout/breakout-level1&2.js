...
const backgroundMusic = new Audio("sprites/background.mp3")
const congratsSound = new Audio("sprites/congrats.mp3")

congratsSound.volume = 1.0;
congratsSound.muted = false;
...
// Function to play congrats sound
function playCongratsSound() {
  congratsSound.play().catch(error => {
      console.error("Error playing congrats sound:", error);
  });
}
...
function newLevel() {
    ctx.font = "25px Arial";
    ctx.fillStyle = "#000000";
    ctx.fillText("Level 2", canvas.width - 685, 25);
    return false; 
}
...
function draw(currentTime) {
  ...
  if (score > 0) {
    ...
    if (score === 105) {
        ...
        newBallActive = true;
        lives = 3;
        showAchievement(); // show achievement notification
    }
    ...
    if (score === 210) {
        window.location.href = "breakout-level3&4.html";
    }
